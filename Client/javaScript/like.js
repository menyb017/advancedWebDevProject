...
const displayProducts = (productsToDisplay = favoris) => {
  ...
  productsToDisplay.forEach((product) => {
    ...
    AddToFavBtn.addEventListener("click", function () {
      // if the client click add the items to favorites and change the number of items in the navbar
      addOrDeleteTofavoritesItems(product);
      getNumofFav();
    });
    AddToCardBtn.addEventListener("click", function () {
      // if the client click add the items to cart
      addToCart(product);
    });
...
function addToCart(product) {
  ...
  if (!produitExistant) {
    ...
    cart = favoris.filter((item) => item.productName !== product.productName); // Remove item from favorites
